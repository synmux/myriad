  ...
  const renderNode = (node: TicketNode) => {
    ...
    return (
      <div
        key={node.ticket.ID}
        className="mb-2"
        style={{ ["--level" as string]: node.level }}
      >
        <div className="relative ml-[calc(24px*var(--level))] flex items-start">
          ...
          >
            <div className="flex items-start justify-between">
              <div>
                <div
                  className={`font-medium text-foreground ${isSynthwaveMode ? "neon-text" : ""}`}
                >
                  <span className="text-primary">{node.ticket.ID}</span>{" "}
                  {node.ticket.Title}
                </div>
                {node.ticket.Project && (
                  <div
              ...
              <div className="flex space-x-2">
                {node.ticket.Status && (
                  <Badge className={getStatusColor(node.ticket.Status)}>
                    {node.ticket.Status}
                  </Badge>
                )}
                {node.ticket.Priority && (
                  <Badge className={getPriorityColor(node.ticket.Priority)}>
                    {node.ticket.Priority}
                  </Badge>
                )}
              </div>
