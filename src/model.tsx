...
export default function Model() {
    ...
    <ActionPanel>
      ...
      <ActionPanel.Section title="Actions">
        <Action
          title={"Export Models"}
          icon={Icon.Upload}
          onAction={() => ExportData(models.data, "Models")}
        />
        <Action
          title={"Import Models"}
          icon={Icon.Download}
  ...
  const filteredModels = sortedModels
    .filter(
      (value, index, self) =>
        index === self.findIndex((model) => model.id === value.id),
    )
    .filter((model) => {
      if (searchText === "") {
        return true;
  ...
  const defaultModelOnly =
    filteredModels.find((x) => x.id === DEFAULT_MODEL.id) ?? DEFAULT_MODEL;
