  ...
  const getActionPanel = (selectedChat: Chat) => (
    <ActionPanel>
      {question.length > 0 ? (
        <PrimaryAction
          title="Get Answer"
          onAction={() => use.chats.ask(question, [], conversation.model)}
        />
      ) : selectedChat.answer ? (
        <>
          <CopyActionSection
      ...
      <FormInputActionSection
        ...
        onSubmit={(question, files) =>
          use.chats.ask(question, files, conversation.model)
        }
        models={models}
      ...
      {sortedChats.map((sortedChat, i) => {
        return (
          <List.Item
            ...
            actions={
              use.chats.isLoading ? undefined : getActionPanel(sortedChat)
            }
          />
        );
      })}
