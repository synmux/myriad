...
export default function CommandView(props: CommandLaunchProps) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { content, error } = await fetchContent(
        requestedCommand.contentSource,
      );
  ...
  if (
    requestedCommand.contentSource === "browserTab" &&
  ...
  return (
    <Detail
      markdown={viewBuilder.buildContent()}
      actions={viewBuilder.buildActionPanel()}
    />
  );
}
...
class CommandViewBuilder {
  ...
  buildContent(): string {
    ...
    if (this.chat.isAborted) {
      ...
      chatWidthPx = 6;
      footerMessage =
        "An unexpected error occurred; the command execution failed.";
...
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${this.iconSizePx}" style="background: transparent;">
  ...
  <text x="${width - messageWidth}" y="14.5" class="text" style="fill: ${color};">${message}</text>
    ...
    return `${modelIcon}![CommandFooter](data:image/svg+xml;base64,${Buffer.from(
      statImage,
      "utf-8",
    ).toString("base64")})`;
  }
  ...
  getFrontmostAppIcon(): string {
    ...
    if (this.command.contentSource === "clipboard") {
      ...
      try {
        appIconPath = getAppIconPath(this.frontmostApp.path).replace(
          / /g,
          "%20",
        );
      } catch (e) {
  ...
  buildActionPanel(): React.JSX.Element {
    ...
    const pasteToActiveApp = (
      <Action.Paste
        ...
        icon={
          this.frontmostApp
            ? { fileIcon: this.frontmostApp.path }
            : Icon.AppWindow
        }
      />
    );
...
const BROWSER_EXTENSION_NOT_AVAILABLE_VIEW = (
  ...
  >
    <List.EmptyView
      ...
      description={
        "This command need install Raycast browser extension to work. Please install it first"
      }
    />
  </List>
);
