...
const visionModelName: string =
  (preferences.useVisionModel && preferences.visionModelName) || "gpt-4o";
  ...
  async function getChatResponse(prompt: string) {
    try {
      ...
      if (!data) {
        ...
        setLoading(false);
        setResponse(
          "## ⚠️ Data couldn't load. Check image selection or clipboard and try again.",
        );
        return;
      }
      ...
      metadata={
        imageMeta.size || imageMeta.width || imageMeta.height ? (
          <Detail.Metadata>
            {imageMeta.size ||
              imageMeta.width ||
              (imageMeta.height && <Detail.Metadata.Separator />)}
            {imageMeta.size && (
              <Detail.Metadata.Label
                title="Size"
                text={toUnit(imageMeta.size)}
              />
            )}
            {imageMeta.width && (
              <Detail.Metadata.Label
                title="Width"
                text={String(imageMeta.width)}
              />
            )}
            {imageMeta.height && (
              <Detail.Metadata.Label
                title="Height"
                text={String(imageMeta.height)}
              />
            )}
            {cumulative_tokens > 0 && (
              <>
                ...
                <Detail.Metadata.Label
                  ...
                  text={cumulative_tokens.toString()}
                />
                <Detail.Metadata.Label
