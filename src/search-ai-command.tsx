...
import CommandView, {
  type CommandLaunchProps,
} from "./views/command/command-view";
...
function SearchAiCommand() {
  ...
  const sortedCommands = Object.values(commands.data).sort((a, b) =>
    a.name.localeCompare(b.name),
  );
  const filteredCommands = sortedCommands
    .filter(
      (value, index, self) =>
        index === self.findIndex((cmd) => cmd.id === value.id),
    )
    .filter((cmd) => {
    ...
    <ActionPanel>
      ...
      <ActionPanel.Section>
        <Action
          ...
          icon={Icon.Pencil}
          onAction={() =>
            navigation.push(<CommandForm cmd={cmd} use={{ commands }} />)
          }
        />
        <Action
        ...
        <Action
          ...
          onAction={() =>
            navigation.push(
              <CommandForm cmd={cmd} isNew={true} use={{ commands }} />,
            )
          }
        />
      ...
      <ActionPanel.Section>
        {!commands.isDefault(cmd.id) && (
          <DestructiveAction
            ...
            dialog={{
              title:
                "Are you sure you want to remove this AI command from your collection?",
            }}
            icon={Icon.Trash}
        ...
        {commands.isDefault(cmd.id) && (
          <DestructiveAction
            ...
            dialog={{
              title:
                "Are you sure you want to reset this action to its default settings?",
            }}
            icon={Icon.Repeat}
        ...
        {filteredCommands
          ...
          .map((cmd) => (
            <List.Item
              ...
              actions={
                selectedCommandId === cmd.id ? getActionPanel(cmd) : undefined
              }
            />
          ))}
      </List.Section>
            ...
            actions={
              <ActionPanel title="Create New">
                <Action
                  ...
                  onAction={() =>
                    navigation.push(
                      <CommandForm name={searchText} use={{ commands }} />,
                    )
                  }
                />
              </ActionPanel>
