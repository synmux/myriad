...
export default function Summarize() {
  ...
  const [separateDefaultModel, setSeparateDefaultModel] = useState<
    Model[] | null
  >(null);

  useEffect(() => {
    if (!modelLoading) {
      setSeparateDefaultModel(
        Object.values(data).filter((x) => x.id !== "default"),
      );
        ...
        <ActionPanel>
          ...
          <Action
            ...
            onAction={() => {
              ...
              push(
                <Ask conversation={conversation} initialQuestion={question} />,
              );
            }}
          />
          {content && <RetryAction />}
