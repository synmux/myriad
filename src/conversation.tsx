...
export default function Conversation() {
  ...
  useEffect(() => {
    setConversation(
      conversations.data.find((x) => x.id === selectedConversationId),
    );
  ...
  const uniqueConversations = conversations.data.filter(
    (value, index, self) =>
      index === self.findIndex((conversation) => conversation.id === value.id),
  );
  ...
  const uniqueSortedConversations =
    pinnedConversation.length > 0
      ? sortedConversations.filter((x) => !x.pinned)
      : sortedConversations;
          ...
          onAction={() =>
            push(
              <ImportForm
                ...
                onSubmit={async (file) => {
                  ImportData<ConversationType>("conversations", file).then(
                    (data) => {
