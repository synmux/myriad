...
export default function Saved() {
    ...
    <ActionPanel>
      ...
      <ActionPanel.Section title="Delete">
        <DestructiveAction
          ...
          dialog={{
            title:
              "Are you sure you want to unsave this answer from your collection?",
          }}
        ...
        <DestructiveAction
          ...
          dialog={{
            title:
              "Are you sure you want to clear all your saved answer from your collection?",
          }}
  ...
  const filteredSavedChats = sortedSavedChats
    .filter(
      (value, index, self) =>
        index === self.findIndex((answer) => answer.id === value.id),
    )
    .filter((answer) => {
      if (searchText === "") {
        return true;
  ...
  return (
    ...
    >
      {savedChat.data.length === 0 ? (
        ...
        <List.Section
          title="Saved"
          subtitle={filteredSavedChats.length.toLocaleString()}
        >
          {filteredSavedChats.map((chat) => (
            <List.Item
              ...
              title={chat.question}
              accessories={[
                { text: new Date(chat.created_at ?? 0).toLocaleDateString() },
              ]}
              detail={<AnswerDetailView chat={chat} />}
              actions={
                chat && selectedAnswerId === chat.id
                  ? getActionPanel(chat)
                  : undefined
              }
            />
