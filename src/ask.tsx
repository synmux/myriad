...
export default function Ask(props: {
  ...
  useEffect(() => {
    ...
    if (
      ...
      (conversation.chats.length > 0 && question.data.length > 0)
    ) {
  ...
  useEffect(() => {
    ...
    if (models.data && conversation.chats.length === 0) {
      const defaultUserModel =
        models.data[DEFAULT_MODEL.id] ?? conversation.model;
      setConversation({
        ...conversation,
  ...
  const getActionPanel = (question: string, model: Model) => (
    <ActionPanel>
      <PrimaryAction
        title="Get Answer"
        onAction={() => chats.ask(question, [], model)}
      />
  ...
  return (
    <List
      ...
      searchBarPlaceholder={
        chats.data.length > 0 ? "Ask another question..." : "Ask a question..."
      }
    >
      <ChatView
        data={chats.data}
