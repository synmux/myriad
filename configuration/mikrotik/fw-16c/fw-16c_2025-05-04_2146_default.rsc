# 2025-05-04 21:45:27 by RouterOS 7.19rc1
# software id = KH3M-8RQA
#
# model = RB5009UPr+S+
# serial number = HE108J2C6WG
/interface bridge
add add-dhcp-option82=yes admin-mac=48:A9:8A:34:3C:D6 auto-mac=no \
    dhcp-snooping=yes igmp-snooping=yes name=br-private port-cost-mode=short
/interface ethernet
set [ find default-name=ether1 ] name=ether1-virgin
set [ find default-name=ether2 ] name=ether2-vodafone
set [ find default-name=ether3 ] name=ether3-raspi
...
/interface wireguard
add comment=back-to-home-vpn listen-port=22092 mtu=1420 name=back-to-home-vpn
/interface vlan
add interface=br-private name=vlan-dave vlan-id=1000
/interface list
add name=iflist-wan
add name=iflist-lan
add name=iflist-cpe
...
/ip dhcp-server
add address-pool=poolv4-br-private-dhcp interface=br-private lease-time=10m \
    name=dhcp-br-private
add address-pool=poolv4-vlan-dave-dhcp interface=vlan-dave lease-time=10m \
    name=dhcp-vlan-dave
/ip smb users
set [ find default=yes ] disabled=yes
/interface l2tp-client
add add-default-route=yes connect-to=194.4.172.12 default-route-distance=30 \
    disabled=no name=l2tp-aaisp profile=default user=dw121@a.2
/routing table
add disabled=no fib name=dave
/zerotier
set zt-central disabled=no disabled=no name=zt-central
/zerotier interface
...
add bridge=br-private interface=ether4-private internal-path-cost=10 \
    path-cost=10
...
add interface=ether3-raspi list=iflist-lan
add interface=ether1-virgin list=iflist-wan
add interface=ether2-vodafone list=iflist-wan
add interface=ether1-virgin list=iflist-cpe
add interface=ether2-vodafone list=iflist-cpe
add interface=zt-backplane list=iflist-vpn
add interface=*C list=iflist-vpn
add interface=vlan-dave list=iflist-lan
add interface=l2tp-aaisp list=iflist-masquerade
add interface=ether1-virgin list=iflist-masquerade
add interface=ether2-vodafone list=iflist-masquerade
add interface=l2tp-aaisp list=iflist-wan
/interface ovpn-server server
add mac-address=FE:33:98:AA:F1:63 name=ovpn-server1
/ip address
add address=10.0.99.1/24 interface=br-private network=10.0.99.0
add address=192.168.100.2/24 interface=ether1-virgin network=192.168.100.0
add address=10.0.98.1/24 interface=vlan-dave network=10.0.98.0
/ip cloud
set back-to-home-vpn=enabled ddns-enabled=yes ddns-update-interval=1h
/ip dhcp-client
add default-route-distance=10 interface=ether1-virgin use-peer-dns=no \
    use-peer-ntp=no
add default-route-distance=20 interface=ether2-vodafone use-peer-dns=no \
    use-peer-ntp=no
/ip dhcp-server lease
add address=10.0.99.10 client-id=1:0:11:32:c9:fd:24 mac-address=\
    00:11:32:C9:FD:24 server=dhcp-br-private
add address=10.0.98.11 client-id=1:24:5e:be:80:2d:21 mac-address=\
    24:5E:BE:80:2D:21 server=dhcp-vlan-dave
add address=10.0.99.11 client-id=1:26:5e:bf:80:2d:21 mac-address=\
    26:5E:BF:80:2D:21 server=dhcp-br-private
/ip dhcp-server network
add address=10.0.98.0/24 dns-server=10.0.98.1 gateway=10.0.98.1 netmask=24 \
    ntp-server=10.0.98.1
add address=10.0.99.0/24 dns-server=10.0.99.1 gateway=10.0.99.1 ntp-server=\
    10.0.99.1
/ip dns
set allow-remote-requests=yes cache-max-ttl=5m cache-size=16384KiB \
    ...
    https://dns.nextdns.io/f5377a verify-doh-cert=yes
/ip dns static
...
add address=90.155.88.111 comment="754t aaisp nat" list=list-v4-trusted
add address=81.187.62.64/27 comment="754t aaisp public" list=list-v4-trusted
add address=81.187.192.60 comment="754t aaisp l2tp" list=list-v4-trusted
add address=81.187.148.148 comment="754t aaisp mobile" list=list-v4-trusted
add address=10.0.101.0/24 comment="754t internal private" list=\
    list-v4-trusted
add address=10.0.99.0/24 comment="internal rvnet" list=list-v4-internal
add address=10.0.98.0/24 comment="internal sl1p" list=list-v4-internal
/ip firewall filter
...
add dst-address=162.159.65.1
/ip upnp
set enabled=yes
/ip upnp interfaces
add interface=ether1-virgin type=external
add interface=br-private type=internal
add interface=vlan-dave type=internal
/ipv6 address
add from-pool=aaisp-dhcpv6 interface=l2tp-aaisp
add from-pool=aaisp-dhcpv6 interface=vlan-dave
/ipv6 dhcp-client
add add-default-route=yes interface=l2tp-aaisp pool-name=aaisp-dhcpv6 \
...
add address=2001:8b0:65d3::/48 comment="754t aaisp full v6 allocation" list=\
    list-v6-trusted
...
/system scheduler
add name=b0rt on-event="/system reboot" policy=\
