...
class FileOrganizer:
    ...
    def _preview_operations(self,
        ...
        for dimension_str, stats in results.items():
            ...
            self.logger.info(f"[DRY RUN] Would {operation.value} {len(stats.files)} files to {dimension_dir}")
    ...
    def _execute_operations(self,
        ...
        try:
            ...
            self.logger.error("Cannot create target directory", 
                            directory=str(target_directory), error=str(e))
            return
        ...
        for dimension_str, stats in results.items():
            ...
            try:
                ...
                self.logger.info("Created dimension directory", directory=str(dimension_dir))
            except OSError as e:
                self.logger.error("Cannot create dimension directory",
                                directory=str(dimension_dir), error=str(e))
            ...
            for file_path in stats.files:
                self._process_single_file(
                    Path(file_path), dimension_dir, operation
                )
    ...
    def _process_single_file(self,
        ...
        try:
            if operation == OperationType.MOVE:
                ...
                try:
                    relative_source = source_path.resolve().relative_to(target_path.parent.resolve())
            ...
            self.logger.debug(f"Successfully {operation.value}d file",
            ...
            self.logger.warning(f"Failed to {operation.value} file",
...
def validate_operation_type(operation_str: str) -> OperationType:
    ...
    try:
        ...
        raise ValueError(f"Invalid operation: {operation_str}. Must be one of: {valid_operations}")
