...
export function TicketDetail({ ticket, allTickets, onTicketSelect, isSynthwaveMode = false }: TicketDetailProps) {
  // Find parent ticket
  ...
  return (
    <div className={`h-full overflow-auto bg-background p-4 ${isSynthwaveMode ? "grid-bg" : ""}`}>
      <div className="mb-4 flex items-center justify-between">
        <h2 className={`text-xl font-semibold ${isSynthwaveMode ? "text-primary neon-text" : "text-primary"}`}>
          Ticket Details
        </h2>
        <Button variant="ghost" size="icon" className="text-foreground hover:bg-destructive/20 hover:text-destructive">
          <X className="h-5 w-5" />
        </Button>
      </div>
      ...
      <div className="space-y-6">
        <div>
          <div className="mb-2 flex items-center justify-between">
            <h3 className={`text-lg font-medium ${isSynthwaveMode ? "text-pink-400" : "text-amber-500"}`}>
              {ticket.ID}
            </h3>
          ...
          <h1 className={`text-2xl font-bold text-foreground ${isSynthwaveMode ? "neon-text" : ""}`}>{ticket.Title}</h1>
        </div>
        ...
        {ticket.Description && (
          <div>
            ...
            <div className={`rounded-md border border-border bg-card ${isSynthwaveMode ? "neon-border" : "p-3"}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            {ticket.Assignee && (
              ...
              >
                <h3
                  className={`mb-1 flex items-center text-sm font-medium ${
                    isSynthwaveMode ? "text-teal-300" : "text-green-700 dark:text-green-400"
                  }`}
                >
            ...
            {ticket.Creator && (
              ...
              >
                <h3
                  className={`mb-1 flex items-center text-sm font-medium ${
                    isSynthwaveMode ? "text-yellow-300" : "text-yellow-700 dark:text-yellow-400"
                  }`}
                >
        ...
        {ticket.Labels && (
          <div>
            <h3
              className={`mb-1 flex items-center text-sm font-medium ${
                isSynthwaveMode ? "text-cyan-300" : "text-yellow-700 dark:text-yellow-400"
              }`}
            >
        ...
        {parentTicket && (
          <div>
            <h3
              className={`mb-1 flex items-center text-sm font-medium ${
                isSynthwaveMode ? "text-pink-300" : "text-amber-700 dark:text-amber-400"
              }`}
            >
