  ...
  if (highLevelTickets.length === 0) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className={`text-muted-foreground ${isSynthwaveMode ? "neon-text" : ""}`}>
          ...
          return (
            <motion.div
              ...
              className={`overflow-hidden rounded-lg border ${
                isSynthwaveMode ? "neon-border border-cyan-500 bg-cyan-900/20" : "border-border bg-card"
              }`}
            >
              <div className={`p-6 ${isSynthwaveMode ? "grid-bg" : ""}`}>
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center">
                    <FileText className={`mr-2 h-5 w-5 ${isSynthwaveMode ? "text-cyan-300" : "text-primary"}`} />
                    <h3 className={`text-lg font-semibold ${isSynthwaveMode ? "neon-text-cyan" : "text-foreground"}`}>
                ...
                <div className="mb-4">
                  <p className={`${isSynthwaveMode ? "text-cyan-100" : "text-muted-foreground"}`}>
                    {ticket.Description}
                  </p>
                </div>
                ...
                <div className="mb-4 flex flex-wrap items-center gap-4 text-sm">
                  <div className="flex items-center">
                    <Layers className={`mr-1 h-4 w-4 ${isSynthwaveMode ? "text-pink-300" : "text-muted-foreground"}`} />
                ...
                {relatedTickets.length > 0 && (
                  <>
                    <div className="flex items-center justify-between">
                      <h4 className={`text-sm font-medium ${isSynthwaveMode ? "text-yellow-300" : "text-foreground"}`}>
                      ...
                      <Button
                        ...
                        onKeyUp={(e) => e.key === "Enter" && setExpandedTicket(isExpanded ? null : ticket.ID)}
