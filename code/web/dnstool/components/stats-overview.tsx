      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{stats.unique_domains.toLocaleString()} unique domains</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{blockedQueries.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">{blockedPercentage}% of total queries</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.unique_ips.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">Active network devices</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.time_range.duration_hours.toFixed(2)} hrs</div>
            <p className="text-xs text-muted-foreground">
              {formatDate(stats.time_range.start)} - {formatDate(stats.time_range.end)}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Query sources in the analyzed logs</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {stats.source_distribution.map((source) => (
                <div key={source.source} className="flex items-center">
                  <div className="w-1/3 font-medium capitalize">{source.source}</div>
                  <div className="w-2/3">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Query statuses in the analyzed logs</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {stats.status_distribution.map((status) => (
                <div key={status.status} className="flex items-center">
                  <div className="w-1/3 font-medium capitalize">{status.status}</div>
                  <div className="w-2/3">
                    <div className="flex items-center gap-2">
                      <div className="h-2 w-full rounded-full bg-muted overflow-hidden">
                        <div
                      ...
                      <span className="text-sm text-muted-foreground">{status.count.toLocaleString()}</span>
                    </div>
                  </div>
                </div>
              ))}
